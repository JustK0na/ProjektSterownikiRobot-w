...
//#include "st_logo1.h"
//#include "st_logo2.h"
#define LAYER_WIDTH  240
#define LAYER_HEIGHT 160
...
uint16_t Layer1_Buffer[SCREEN_WIDTH * SCREEN_HEIGHT];
uint16_t Layer2_Buffer[SQUARE_SIZE * SQUARE_SIZE];
...
int main(void)
{
  ...
  for (int i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++) {
      Layer1_Buffer[i] = 0xF800;
  }

  for (int i = 0; i < SQUARE_SIZE * SQUARE_SIZE; i++) {
      Layer2_Buffer[i] = 0xe9f4;
  }
  ...
  { 
	y += direction * 4;
	if (y > SCREEN_HEIGHT - SQUARE_SIZE || y < 0) {
	        direction *= -1;
	}
	HAL_LTDC_SetWindowPosition_NoReload(&LtdcHandle, 80, y, 1);
	ReloadFlag = 0;
...
/* Layer1 Configuration ------------------------------------------------------*/
  ...
  pLayerCfg.FBStartAdress = (uint32_t)Layer1_Buffer;
...
/* Layer2 Configuration ------------------------------------------------------*/
  ...
  /* Windowing configuration */
  pLayerCfg1.WindowX0 = 80;
  pLayerCfg1.WindowX1 = 80+SQUARE_SIZE;
  pLayerCfg1.WindowY0 = 0;
  pLayerCfg1.WindowY1 = SQUARE_SIZE;
  ...
  pLayerCfg1.FBStartAdress = (uint32_t)Layer2_Buffer;
  /* Alpha constant (255 totally opaque) */
  ...
  /* Configure the number of lines and number of pixels per line */
  pLayerCfg1.ImageWidth = SQUARE_SIZE;
  pLayerCfg1.ImageHeight = SQUARE_SIZE;
